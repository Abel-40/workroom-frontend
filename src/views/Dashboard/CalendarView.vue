...
const viewMonth = ref(startOfMonth(new Date(eventStore.events[0]?.date || Date.now())));
...
const eventsFor = (day: Date) =>
  eventStore.events.filter((e) => isSameDay(new Date(e.date), day));
        ...
        >
          ...
          <div class="mt-1 space-y-1">
            <div
              ...
              :class="colorDot[event.color]"
              :title="event.title"
            >
              <p class="truncate font-medium">{{ event.title }}</p>
              <span class="flex items-center gap-1 text-subtle">
                {{ event.duration }}
                <component :is="event.priority === 'up' ? ArrowUp : ArrowDown" class="h-2.5 w-2.5" />
              </span>
            </div>
