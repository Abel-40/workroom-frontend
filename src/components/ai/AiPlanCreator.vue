...
import type { MentionItem } from "@/components/ai/MentionTextarea.vue";
...
const removeMember = (id: string) => {
  mentionedMembers.value = mentionedMembers.value.filter((m) => m.id !== id);
};
  ...
  <div class="space-y-4">
    ...
    <template v-else>
      <AiComposer
        ...
        :mentioned-members="mentionedMembers"
        :generating="!!planInFlight"
        :disabled="!prompt.trim()"
        @mention-project="onMentionProject"
        @mention-member="onMentionMember"
        @remove-project="emit('request-project-change', null)"
        @remove-member="removeMember"
        @generate="requestPlan"
      />
      ...
      <template v-if="generation && generation.generatedTasks.length">
        ...
        <AiPlanFlow :tasks="generation.generatedTasks" :project-id="projectId!" @select="openReview" />

        <div class="sticky bottom-4 z-10 flex gap-2 rounded-2xl border border-gray-100 bg-white/95 p-2.5 shadow-lg backdrop-blur-sm">
