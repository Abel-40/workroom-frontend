...
// eligible-assignees endpoint actually returns (department/name) -- no
// workload/open-task figures are shown since nothing computes them yet, and
// inventing a number here would violate "never invent data."
      ...
      <div class="flex flex-col gap-4 p-6 pb-5">
        <div class="flex flex-col gap-1">
          ...
          <span class="flex items-center gap-2 text-sm text-subtle">
            Members eligible on
            <span class="inline-flex items-center gap-1.5 rounded-lg bg-info px-2.5 py-1 text-xs font-medium text-info-foreground">
              {{ projectName }}
            </span>
          </span>
        </div>
        ...
        <div class="flex items-center gap-2.5">
          ...
          <Select v-model="sort">
            <SelectTrigger class="w-[170px] gap-2 rounded-xl text-sm">
        ...
        <div class="flex max-h-[390px] flex-col gap-3.5 overflow-y-auto pr-1">
          ...
          <div v-for="group in groups" :key="group.title" class="flex flex-col gap-2">
            <div class="flex items-center gap-2.5 px-0.5">
              <span class="text-[10.5px] font-semibold uppercase tracking-wide text-subtle">{{ group.title }}</span>
            ...
            >
              ...
              <span class="min-w-0 flex-1">
                ...
                <span class="block truncate text-xs text-subtle">{{ member.roleLabel || "Member" }}</span>
              </span>
            </button>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3 pt-1">
          ...
          <span class="text-xs text-subtle">{{ selectedMembers.length }} assignee(s) selected</span>
          <div class="flex-1" />
