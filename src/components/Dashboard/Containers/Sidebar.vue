...
const navItems = [
  ...
  { title: "Department & teams", sectionName: "departments" },
...
<template>
 <div
  class="fixed    bg-white flex flex-col transition-all duration-200 overflow-x-hidden md:z-40 z-[10] overflow-y-hidden "
  :class="[
    isCollapsed ? 'w-20' : windowWidth < 768 ? 'w-40' : 'w-64',
    windowWidth < 768 ? 'left-4' : 'left-5',
    'md:block',
...
>
    ...
    <div class="flex flex-col bg-white pl-4 md:py-2 -ml-5 md:ml-0 lg:h-[80%] flex-1 overflow-y-auto overflow-x-hidden md:justify-around mt-6 absolute ">
      <div class="w-full flex justify-between items-center px-2" :class="isCollapsed?'flex justify-center':''">
        <p v-if="!isCollapsed">Home</p>
        <button
          @click="toggleCollapse"
          class="hidden md:flex items-center  w-10 h-10 sticky top-0 z-10"
        >
      ...
      <div class="absolute inset-0 opacity-20 pointer-events-none overflow-hidden">
        <div class="absolute top-8 right-2 w-[80px]  md:w-[150px] h-[100px] md:h-[80px] bg-blue-400 rounded-full blur-xl"></div>
      ...
      <nav class="space-y-2 lg:h-[75%] w-full">
        <div v-for="(navItem, index) in navItems" :key="index">
          ...
          >
            <a class="w-full lg:min-w-[210px] h-full flex items-center  sm:justify-center md:justify-start gap-2 py-2 px-3 lg:rounded-l-xl transition-all duration-200 sm:w-24"
                :class="[setItemClass(navItem.sectionName), isCollapsed || windowWidth < 768 ? 'justify-start' : 'justify-start']">
              ...
              />
             <span  class="text-xs md:text-base" v-if="!isCollapsed && windowWidth >= 768">{{ navItem.title }}</span>
             <span  class="text-xs md:text-base md:hidden" >{{ navItem.title }}</span>
            </a>
            <div
