    ...
    <nav class="w-full bg-white border-b border-gray-200 shadow-lg shadow-blue-100/40 backdrop-blur-sm">
      <div class="relative flex items-center justify-between px-4 py-2 md:px-8 h-20">
        ...
        <div class="absolute inset-0 opacity-20 pointer-events-none overflow-hidden z-0">
          <div class="absolute right-0 w-1/2 h-9 bg-blue-400 blur-xl rounded-full"></div>
          <div class="absolute left-0 w-1/2 h-9 bg-purple-400 blur-xl rounded-full"></div>
        </div>
        ...
        <!-- Mobile Toggle -->
        <button @click="showNav = !showNav" class="md:hidden text-blue-600 focus:outline-none z-10">
          <Menu class="w-6 h-6" />
        </button>

        <!-- Desktop Nav -->
        <div class="hidden md:flex md:gap-6 md:items-center z-10">
          <a
            v-for="item in navItems"
            :key="item.name"
            :href="item.route"
            class="text-sm font-medium text-gray-700 hover:text-blue-600 px-2 py-1 rounded-md transition-all duration-200 hover:bg-blue-50"
          >
            {{ item.name }}
          </a>
        </div>
        ...
        <div class="hidden md:flex items-center gap-4 z-10">
          <Button variant="ghost" size="icon" class="bg-white shadow-sm relative hover:bg-blue-50 transition-all">
          ...
          <Button variant="ghost" size="icon" class="bg-white shadow-sm hover:bg-blue-50 transition-all">
            <MessageCircle class="w-5 h-5 text-gray-700" />
          </Button>

          <DropdownMenu>
            <DropdownMenuTrigger class="focus:outline-none">
              <div class="flex items-center gap-2 bg-white px-2 py-1 rounded-lg shadow-sm cursor-pointer hover:bg-blue-50 transition-all">
      ...
      <transition name="slide-down">
        <div
          v-show="showNav"
          class="md:hidden w-full bg-white flex flex-col items-center gap-2 py-4 overflow-hidden z-10"
        >
          <a
            v-for="item in navItems"
            :key="item.name"
